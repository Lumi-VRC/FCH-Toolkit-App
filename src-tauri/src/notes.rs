...
pub fn load_all_notes() -> UserNotes {
	...
	if let Ok(data) = fs::read(&p) {
		if let Ok(n) = serde_json::from_slice::<UserNotes>(&data) { return n; }
	}
	UserNotes::default()
}
...
pub fn add_note(user_id: String, text: String) -> Result<(), String> {
	...
	if user_id.trim().is_empty() { return Err("user_id required".into()); }
...
pub fn get_all_notes() -> Result<serde_json::Value, String> {
	...
	let all = load_all_notes();
	Ok(serde_json::json!({ "notes": all.notes, "usernames": all.usernames, "watchlist": all.watchlist }))
}
...
pub fn get_note(user_id: String) -> Result<serde_json::Value, String> {
	...
	let all = load_all_notes();
	let text = all.notes.get(&user_id).and_then(|v| v.last()).map(|n| n.text.clone()).unwrap_or_default();
...
pub fn set_username(user_id: String, username: String) -> Result<(), String> {
	if user_id.trim().is_empty() { return Err("user_id required".into()); }
	let mut all = load_all_notes();
	let effective = if username.trim().is_empty() { "Not Yet Recorded".to_string() } else { username };
